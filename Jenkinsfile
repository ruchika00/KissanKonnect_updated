...
"""
    ...
    environment {
        DOCKER_IMAGE = "sql-optimizer-app"
    ...
    stages {
        ...
        stage('Checkout Code') {
            steps {
                deleteDir()
                sh "git clone https://github.com/RuchikaPardeshi/SQL-Query-Optimizer.git ."
        ...
        stage('SonarQube Analysis') {
            steps {
                container('sonar-scanner') {
                    sh """
                        sonar-scanner \
                        -Dsonar.projectKey=2401146_sql_optimizer \
                        -Dsonar.projectName=2401146_sql_optimizer \
    ...
    post {
        success { echo "🎉 SQL Query Optimizer CI/CD Pipeline completed successfully!" }
